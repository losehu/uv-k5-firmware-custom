...
void ADC_Configure(ADC_Config_t *pAdc) {
    ...
    SYSCON_CLK_SEL = (ADC_GetClockConfig() & ~SYSCON_CLK_SEL_W_SARADC_SMPL_MASK) |
                     ((pAdc->CLK_SEL << SYSCON_CLK_SEL_W_SARADC_SMPL_SHIFT) & SYSCON_CLK_SEL_W_SARADC_SMPL_MASK);
    ...
    if (pAdc->CALIB_OFFSET_VALID) {
        ...
        SARADC_CALIB_OFFSET =
                (SARADC_CALIB_OFFSET & ~SARADC_CALIB_OFFSET_VALID_MASK) | SARADC_CALIB_OFFSET_VALID_BITS_NO;
    }

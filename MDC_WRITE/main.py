...
max_contact = 15
line_count = 0
MDC_ADD = [0x1D48, 0x1D88, 0x1DC8, 0x1FA8]
MDC_NUM_ADD = 0X1D00
file_out = "MDC.txt"
com_open = ""
...
Crc16Tab = [0, 4129, 8258, 12387, 16516, 20645, 24774, 28903, 33032, 37161, 41290, 45419, 49548, 53677, 57806, 61935,
            4657, 528, 12915, 8786, 21173, 17044, 29431, 25302,
            37689, 33560, 45947, 41818, 54205, 50076, 62463, 58334, 9314, 13379, 1056, 5121, 25830, 29895, 17572, 21637,
            42346, 46411, 34088, 38153, 58862, 62927, 50604, 54669, 13907,
            9842, 5649, 1584, 30423, 26358, 22165, 18100, 46939, 42874, 38681, 34616, 63455, 59390, 55197, 51132, 18628,
            22757, 26758, 30887, 2112, 6241, 10242, 14371, 51660, 55789,
            ...
            19684, 23749, 11298, 15363, 3168, 7233, 60846, 64911, 52716, 56781, 44330, 48395, 36200, 40265, 32407,
            28342, 24277, 20212, 15891, 11826, 7761, 3696, 65439, 61374,
            57309, 53244, 48923, 44858, 40793, 36728, 37256, 33193, 45514, 41451, 53516, 49453, 61774, 57711, 4224, 161,
            12482, 8419, 20484, 16421, 28742, 24679, 33721, 37784, 41979,
            ...
            35305, 47498, 43435, 22596, 18533, 30726, 26663, 6336, 2273, 14466, 10403, 52093, 56156, 60223, 64286,
            35833, 39896, 43963, 48026, 19061, 23124, 27191, 31254, 2801,
            6864, 10931, 14994, 64814, 60687, 56684, 52557, 48554, 44427, 40424, 36297, 31782, 27655, 23652, 19525,
            15522, 11395, 7392, 3265, 61215, 65342, 53085, 57212, 44955,
    ...
    with open(file_out, 'w+') as file:
        ...
        with serial.Serial(com_open, 38400, timeout=1) as ser:
            ...
            while a < num_contact:
                ...
                payload = b'\x1B\x05' + b'\x08\x00' + add.to_bytes(2,
                                                                   byteorder='little') + b'\x10\x00' + b'\x82\x40\x74\x65'
                # 将 payload 中的最后四个字节替换为当前时间戳
                hex_string = ' '.join(['{:02X}'.format(byte) for byte in payload])
                crc = crc16_ccitt(payload)
                payload = payload + bytes([crc & 0xFF, ]) + bytes(
                    [crc >> 8, ])  # swap bytes of crc to get little endian
                message = b'\xAB\xCD' + b'\x0C\x00' + payload_xor(payload) + b'\xDC\xBA'
                ser.write(message)
                full_response = ser.read(128)
                if len(full_response) == 0:
...
def write_eeprom_byte(add, num):
    ...
    with serial.Serial(com_open, 38400, timeout=1) as ser:
        payload = b'\x1D\x05' + b'\x09\x00' + add.to_bytes(2,
                                                           byteorder='little') + b'\x0F\x00' + b'\x82\x40\x74\x65' + num.to_bytes(
            1, byteorder='big')
        # 将 payload 中的最后四个字节替换为当前时间戳
        crc = crc16_ccitt(payload)
        payload = payload + bytes([crc & 0xFF, ]) + bytes([crc >> 8, ])  # swap bytes of crc to get little endian
...
if __name__ == "__main__":
    ...
    # 如果用户没有输入串口信息，则使用默认可用串口
    if not com_open:
        com_open = get_default_serial_port()

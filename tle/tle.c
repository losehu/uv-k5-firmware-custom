    ...
    switch (mode) {
        ...
        case TIME: {
            ...
            for (uint8_t i = 0; i < 6; i++) {
                ...
                if (freqInputString[i + 6] != '-' && freqInputString[i + 6] != 0)
                    show2[table[i + 6]] = freqInputString[i + 6];
            }
...
#ifdef ENABLE_RTC
    if (my_time[0] >= 25 && my_time[0] <= 99 && my_time[1] <= 12 && my_time[2] <= 31\
        && my_time[3] <= 24 && my_time[4] <= 60 && my_time[5] <= 60) {
        uint8_t store_time[6];
        DS3231_GetTime(store_time);
        if (compare_times(my_time, store_time))
            DS3231_SetTime(my_time);
        else
            memcpy(my_time, store_time, 6);
    } else {
